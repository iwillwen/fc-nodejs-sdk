...
const debug = debug_1.default('lambda');
function signString(source, secret) {
    const buffer = sha256_1.default(source, secret);
    return enc_base64_1.default.stringify(buffer);
}
...
function genUserAgent(defaultUA) {
    if (defaultUA) {
        return defaultUA;
    }
    if (process) {
...
class Client {
    ...
    async request(method, path, query, body, headers = {}) {
        ...
        if (body) {
            ...
            if (buffer_1.Buffer.isBuffer(body)) {
                ...
                buff = new buffer_1.Buffer(body, 'utf8');
                headers['content-type'] = 'application/octet-stream';
            }
                ...
                buff = new buffer_1.Buffer(JSON.stringify(body), 'utf8');
                headers['content-type'] = 'application/json';
            }
            if ('function' !== typeof body.pipe) {
                const md5 = enc_base64_1.default.stringify(md5_1.default(buff.toString('binary')));
        ...
        if ((res.status < 200 || res.status >= 300) && data !== null) {
            ...
            const requestid = res.headers['x-fc-request-id'];
    ...
    getService(serviceName, headers = {}, qualifier) {
        return this.get(`/services/${getServiceName(serviceName, qualifier)}`, undefined, headers);
    }
    /**
     * 更新Service信息
     *
    ...
    createFunction(serviceName, options, headers) {
        options = this.normalizeParams(options);
    ...
    getFunction(serviceName, functionName, headers = {}, qualifier) {
        return this.get(`/services/${getServiceName(serviceName, qualifier)}/functions/${functionName}`, undefined, headers);
    }
    /**
     * 获取Function Code信息
     *
    ...
    invokeFunction(serviceName, functionName, event, headers, qualifier) {
        if (event && typeof event !== 'string' && !buffer_1.Buffer.isBuffer(event)) {
        ...
        return this.post(path, event, headers, undefined);
    }
    /**
     * 创建Trigger
     *
    ...
    untagResource(resourceArn, tagKeys, all = false, options = {}, headers) {
        ...
        options.all = all;
        return this.request('DELETE', '/tag', undefined, options, headers);
    }
    /**
     * 获取某个资源的所有tag
     *
    ...
    getProvisionConfig(serviceName, functionName, qualifier, headers) {
        return this.get(`/services/${getServiceName(serviceName, qualifier)}/functions/${functionName}/provision-config`, undefined, headers);
    }
    /**
    ...
    static getSignature(accessKeyID, accessKeySecret, method, path, headers, queries) {
        const stringToSign = helper_1.composeStringToSign(method, path, headers, queries);
